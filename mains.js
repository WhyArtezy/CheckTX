...
const USDC_CONTRACT = "0xceba9300f2b948710d2de5d0d3e7d6b7f0d5f8eb";

const USDC_WALLETS = {
  EURO: "0x65cc602e616ca786bdb4bab00a6272060f0082fb",
  US:   "0x74667d9edd871150ce38ebc26355758ba31f44b5"
};
...
function loadCache() {
  try {
    if (fs.existsSync(CACHE_FILE)) {
      ...
      lastMessageCache     = data.lastMessageCache     || null;
      lastUsdcMessageCache = data.lastUsdcMessageCache || null;
    }
  } catch {
    lastMessageCache     = null;
    lastUsdcMessageCache = null;
  }
}
...
function initProvider() {
  ...
  contractUsdc = new ethers.Contract(USDC_CONTRACT, ABI, provider);
}
...
async function sendOrEditUsdc(chatId, text) {
  try {
    if (!users[chatId].usdcMessageId) {
      const res = await axios.post(
        `https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`,
        {
          chat_id: chatId,
      ...
      users[chatId].usdcMessageId = res.data.result.message_id;
      saveUsers();
    } else {
      await axios.post(
        `https://api.telegram.org/bot${TELEGRAM_TOKEN}/editMessageText`,
        {
          chat_id: chatId,
          message_id: users[chatId].usdcMessageId,
          text,
          parse_mode: "HTML",
          disable_web_page_preview: true
        }
  ...
  try {
    ...
    for (const update of res.data.result) {
      ...
      if (update.message?.text === "/start") {
        ...
        // Hapus pesan USDT lama
        if (users[chatId]?.messageId) {
          await deleteMessage(chatId, users[chatId].messageId);
        }

        // Hapus pesan USDC lama
        if (users[chatId]?.usdcMessageId) {
          await deleteMessage(chatId, users[chatId].usdcMessageId);
        }

        await deleteMessage(chatId, userMsgId);

        // Reset dengan kedua messageId
    ...
    console.error("[checkTelegram ERROR]", err.message);
  }
}
...
async function checkTransfers() {
  ...
  // Fetch USDC balances
  const usdcDecimals = await contractUsdc.decimals();
  ...
  const usdcEuro = parseFloat(ethers.formatUnits(usdcEuroRaw, usdcDecimals)).toFixed(2);
  const usdcUs   = parseFloat(ethers.formatUnits(usdcUsRaw,   usdcDecimals)).toFixed(2);
